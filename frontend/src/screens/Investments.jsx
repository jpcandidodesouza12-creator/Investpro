    ...
    <div style={{ ...P.invCard, borderColor:`${t.color}55` }} className="card-hover">
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start" }}>
        <div style={{ display:"flex", alignItems:"center", gap:10 }}>
          <span style={{ width:36, height:36, borderRadius:10, background:`${t.color}22`, color:t.color, display:"flex", alignItems:"center", justifyContent:"center", fontSize:18, flexShrink:0 }}>
            {t.icon}
          </span>
          <div>
      ...
      {irAlert && (
        <span style={{ ...P.pill, borderColor: irAlert.restam <= 7 ? "#ef444466" : "#F5C51866", color: irAlert.restam <= 7 ? "#ef4444" : "#F5C518" }}>
          ⚠️ IR: {irAlert.aliqAtual.toFixed(1)}%→{irAlert.aliqProx.toFixed(1)}% em {irAlert.restam}d
        </span>
      )}

      <div style={P.metrics}>
        <Met label={t.currency ? `Valor (${t.currency})` : "Investido"} value={t.currency ? `${t.currency} ${inv.valor.toFixed(2)}` : fmt(inv.valor)} />
